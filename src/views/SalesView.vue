...
<template>
  <section class="page">
    <template v-if="session">
      <div class="title-row"><div><p class="eyebrow">เปิดร้าน</p><h1>{{ session.name }}</h1></div><div class="list-actions"><button class="hold-cancel" :style="{ '--hold-progress': `${cancelHoldProgress}%` }" @pointerdown="startCancelHold" @pointerup="clearCancelHold" @pointerleave="clearCancelHold" @pointercancel="clearCancelHold" @contextmenu.prevent><Icon name="X" :size="18" /> กดค้างเพื่อยกเลิก</button><button class="danger" @click="confirmCloseSession">ปิดการขาย</button></div></div>
