  ...
  <section class="page">
    <template v-if="session">
      ...
      <div v-if="activeOrder" class="order">
        ...
        <div v-if="!activeOrder.lines.length" class="empty">ยังไม่มีสินค้า<br><button class="primary" @click="openProductPicker">＋ เพิ่มสินค้า</button></div>
        <article v-for="line in activeOrder.lines" :key="line.id" class="sale-line">
          <div class="sale-line-header">
            ...
            <button v-if="!activeOrder.closed" class="icon" aria-label="ลบสินค้า" @click="call(() => store.removeLine(activeOrder!.id, line.id))">×</button>
          </div>
        ...
        <button v-if="!activeOrder.closed" class="secondary wide add-item-button" @click="openProductPicker">＋ เพิ่มสินค้า</button><button v-if="!activeOrder.closed" class="primary wide" @click="confirmCloseOrder">✓ ปิดการขาย {{ money(activeOrder.lines.reduce((sum, line) => sum + line.total, 0)) }}</button><button v-if="!activeOrder.closed" class="danger-outline wide" @click="confirmRemoveCustomer(activeOrder.id, activeOrder.customerName)">ลบลูกค้าออกจากรายการ</button><template v-else><button class="secondary wide" @click="confirmReopenOrder">↶ เปิดแก้ไขการขาย</button><button class="secondary wide" @click="printReceipt">🖨 พิมพ์ใบเสร็จ / บันทึก PDF</button></template>
      </div>
