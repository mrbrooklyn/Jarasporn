...
<template>
  <section class="page">
    ...
    <button v-for="session in sessions" :key="session.id" class="history-card" @click="selected = session"><div><strong>{{ session.name }}</strong><small>{{ dateThai(session.openedAt) }} · {{ session.orders.length }} ลูกค้า · {{ thaiQuantity(quantity(session)) }}</small></div><div class="history-total"><strong>{{ money(total(session)) }}</strong><small>ดูรายละเอียด ›</small></div></button>
